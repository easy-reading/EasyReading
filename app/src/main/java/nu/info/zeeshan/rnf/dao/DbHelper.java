    ...
    */
    public List<Item> getFacebookFeeds(boolean latestOnly) {
        ...
        if (c.moveToFirst()) {
            do {
                ...
                item.setState(c.getShort(c.getColumnIndexOrThrow(DbStructure.FeedTable.COLUMN_STATE)));
    ...
    public void markAllAs(int state, int type) throws InvalidStateException, InvalidTypeException {
        if (state != DbConstants.State.READ && state != DbConstants.State.UNREAD)
            ...
            values.put(DbStructure.FeedTable.COLUMN_STATE, state);
            if (type == DbConstants.Type.NEWS)
                db.update(DbStructure.NewsFeedTable.TABLE_NAME, values, null, null);
            else if (type == DbConstants.Type.FB)
                db.update(DbStructure.FacebookFeedTable.TABLE_NAME, values, null, null);
            db.close();
        }
    }

    public List<Item> getTopUnread() {
        List<Item> unread = null;
